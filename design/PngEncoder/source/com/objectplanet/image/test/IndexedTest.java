...
public class IndexedTest {
	...
	private static PngEncoder objectplanet_png_encoder = new PngEncoder(PngEncoder.COLOR_INDEXED);
	...
	public void runTest(boolean antialiased) {
		...
		chart.setTitle("Encoding Indexed Image");
		...
		chart.setChartBackground(new Color(231, 221, 231));
		...
		try {
			...
			f.add("Center", chart);
			f.pack();
			f.setBounds(0, ImageEncoderTest.FRAME_SIZE.height * 2, ImageEncoderTest.FRAME_SIZE.width, ImageEncoderTest.FRAME_SIZE.height);
			f.setVisible(true);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			...
			Thread.sleep(1000);
			Image image = getImage(chart, size);
			String path = "images/indexed" + (antialiased ? "_antialiased" : "") + ".png";
	...
	public static void main(String[] argv) {
		IndexedTest test = new IndexedTest();
		test.runTest();
	}
}
